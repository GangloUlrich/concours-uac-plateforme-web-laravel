            ...
            <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
                <div class="container-fluid">
                    <div class="navbar-wrapper">
                        <a class="navbar-brand" href="javascript:;">Utilisateurs</a>
                    </div>
                    ...
                    <div class="row">
                                            ...
                                            @foreach($users as $use)
                                                @if($use->statut == 'E')
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{$use->etudiant()->first()->prenom}}
                                                        </td>
                                                        <td>
                                                            {{$use->email}}
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->etudiant()->first()->contact))

                                                            @else
                                                                {{$use->etudiant()->first()->contact}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->etudiant()->first()->age))

                                                            @else
                                                                {{$use->etudiant()->first()->age}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->etudiant()->first()->entite))

                                                            @else
                                                                {{$use->etudiant()->first()->entite}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->etudiant()->first()->niveau))

                                                            @else
                                                                {{$use->etudiant()->first()->niveau}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->etudiant()->first()->competences))

                                                            @else
                                                                {{$use->etudiant()->first()->competences}}
                                                            @endif
                                                        </td>
                                                        <td class="td-actions text-right">
                                                            <a href="{{route('view_utilisateur', ['n' => $use->id])}}" type="button" rel="tooltip" title="Voir plus" class="btn btn-success btn-link btn-sm">
                                                                <i class="material-icons fas fa-eye"></i>
                                                            </a>
                                                            <a href="{{route('edit_utilisateur', ['n' => $use->id])}}" type="button" rel="tooltip" title="Modifier" class="btn btn-warning btn-link btn-sm">
                                                                <i class="material-icons">edit</i>
                                                            </a>
                                                            <a href="{{route('delete_utilisateur', ['n' => $use->id])}}" type="button" rel="tooltip" title="Supprimer" class="btn btn-danger btn-link btn-sm">
                        ...
                        <div class="col-md-12">
                            <div class="card card-plain">
                                <div class="card-header card-header-success">
                                    <h4 class="card-title mt-0"> Liste des Recruteurs</h4>
                                    <p class="card-category"> Here is a subtitle for this table</p>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                            ...
                                            @foreach($users as $use)
                                                @if($use->statut == 'R')
                                                    <tr>
                                                        ...
                                                        <td>
                                                            {{$use->recruteur()->first()->prenom}}
                                                        </td>
                                                        <td>
                                                            {{$use->email}}
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->recruteur()->first()->contact))

                                                            @else
                                                                {{$use->recruteur()->first()->contact}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->recruteur()->first()->nom_entreprise))

                                                            @else
                                                                {{$use->recruteur()->first()->nom_entreprise}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->recruteur()->first()->domaine))

                                                            @else
                                                                {{$use->recruteur()->first()->domaine}}
                                                            @endif
                                                        </td>
                                                        <td class="">
                                                            @if(is_null($use->recruteur()->first()->poste))

                                                            @else
                                                                {{$use->recruteur()->first()->poste}}
                                                            @endif
                                                        </td>
                                                        <td class="td-actions text-right">
                                                            <a href="{{route('view_utilisateur', ['n' => $use->id])}}" type="button" rel="tooltip" title="Voir plus" class="btn btn-success btn-link btn-sm">
